...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if UIApplication.isFirstLaunch(){
            self.userNotLoggedInSetup()
        }else{
            let userModel = Helper.getLoggedinUser()
            if userModel != nil{
                self.userLoggedInSetup()
            }else{
                self.userNotLoggedInSetup()
            }
        }
    ...
    func userLoggedInSetup(){
        ...
        let viewController = Storyboard.instantiateViewController(withIdentifier: "SetLocationVC") as! SetLocationVC
